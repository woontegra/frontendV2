/**
 * Vardiya 24/48 - Yıllık izin / çalışılmayan günler paneli (izole kopya)
 */
...
export interface VardiyaYillikIzinPanelProps {
...
export function VardiyaYillikIzinPanel({
  exclusions,
  setExclusions,
  success = () => {},
  showToastError = () => {},
}: VardiyaYillikIzinPanelProps) {
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-600 overflow-hidden bg-gray-50/50 dark:bg-gray-900/30 shadow-sm">
      ...
      {isOpen && (
        <div className="px-4 pb-4 pt-0 space-y-3 border-t border-gray-200 dark:border-gray-600">
            ...
            <div className="space-y-1">
              ...
              {exclusions.map((ex, idx) => (
                <div key={ex.id} className="grid grid-cols-12 gap-1 items-center">
                  ...
                  <input type="date" value={ex.start} onChange={(e) => setExclusions((arr) => arr.map((r, i) => (i === idx ? { ...r, start: e.target.value } : r)))} className={`col-span-3 ${inputCls} py-1.5 text-xs`} />
                  <input type="date" value={ex.end} onChange={(e) => setExclusions((arr) => arr.map((r, i) => (i === idx ? { ...r, end: e.target.value } : r)))} className={`col-span-3 ${inputCls} py-1.5 text-xs`} />
                  <input type="number" value={ex.days || ""} onChange={(e) => setExclusions((arr) => arr.map((r, i) => (i === idx ? { ...r, days: Number(e.target.value) || 0 } : r)))} className={`col-span-2 ${inputCls} py-1.5 text-xs`} />
          ...
          <p className="text-xs text-gray-500">Düşüm, girdiğiniz gün sayısına göre yapılır.</p>
        </div>
      )}
